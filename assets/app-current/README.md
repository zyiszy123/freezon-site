...
除特别注明外，这些 PNG 由固定模拟器上的 Debug production SwiftUI 路由采集，使用深色系统外观：
...
| `backup-auto-zh-Hans.png` | — | 用户提供的真实加密备份与自动备份状态画面；用于官网展示 |
...
网页使用已确认的当前 App UI；其中 `backup-auto-zh-Hans.png` 是用户提供的真实备份页面截图。未经授权的照片素材不会被加入页面。
